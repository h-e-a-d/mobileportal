...
class Analytics {
    constructor() {
        ...
        if (window.logger) {
            window.logger.info('[Analytics] Module initialized');
        }
    }
    ...
    push(data) {
        try {
            if (typeof window.dataLayer !== 'undefined') {
                ...
                if (window.logger) {
                    window.logger.debug('[Analytics] Event tracked:', data.event);
                }
            } else if (window.logger) {
                window.logger.warn('[Analytics] GTM dataLayer not available');
            }
        } catch (error) {
            if (window.logger) {
                window.logger.error('[Analytics] Error pushing to dataLayer:', error);
            }
        }
    }
