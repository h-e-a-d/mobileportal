...
class DOMHelper {
    ...
    static createGameCard(game, isFavorite = false, onClick, onFavoriteClick) {
        ...
        // Image error handling with retry and fallback
        img.onerror = function() {
            // Retry loading the image once before falling back to placeholder
            if (retryCount < maxRetries && thumbnail && !thumbnail.startsWith('data:')) {
                retryCount++;
                // Add cache-busting parameter for retry
                const separator = thumbnail.includes('?') ? '&' : '?';
                this.src = `${thumbnail}${separator}retry=${retryCount}`;

                if (window.logger) {
                    window.logger.debug('[DOMHelper] Retrying image load:', thumbnail, 'attempt:', retryCount);
                }
                return;
            }
            ...
            try {
                if (window.gamesManager && typeof window.gamesManager.generatePlaceholderImage === 'function') {
                    this.src = window.gamesManager.generatePlaceholderImage(game.title);
                } else {
                    // Fallback: Generate a simple placeholder directly
                    const initial = game.title ? game.title[0].toUpperCase() : 'G';
                ...
                if (window.logger) {
                    window.logger.warn('[DOMHelper] Image load failed after retries, using placeholder:', thumbnail);
                }
            } catch (error) {
                if (window.logger) {
                    window.logger.error('[DOMHelper] Error in image fallback handler:', error);
                }
            }
        };
