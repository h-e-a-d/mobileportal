...
class Router {
        ...
        for (let i = 0; i < patternParts.length; i++) {
            ...
            if (patternPart.startsWith(':')) {
                ...
                try {
                    ...
                    // Apply Sanitizer for slugs if available
                    if (paramName === 'slug' && typeof window.Sanitizer !== 'undefined') {
                        decodedValue = window.Sanitizer.sanitizeSlug(decodedValue);
                    }

                    // Validate the parameter
                    if (!this.validateRouteParam(paramName, decodedValue)) {
                        return null;
                    }
                    ...
                    if (window.logger) {
                        window.logger.warn('[Router] URI decode error:', e);
                    }
                    return null;
                }
    ...
    /**
     * Validate a route parameter by name
     * @param {string} paramName - Parameter name (slug, category, etc.)
     * @param {string} value - Parameter value to validate
     * @returns {boolean} Whether the parameter is valid
     */
    validateRouteParam(paramName, value) {
        if (!value) {
            return false;
        }

        // Max length per parameter type
        const maxLengths = { slug: 100, category: 50 };
        const maxLength = maxLengths[paramName] || 100;

        // Slug and category share the same alphanumeric-hyphen pattern
        if (paramName === 'slug' || paramName === 'category') {
            if (!/^[a-z0-9]([a-z0-9-]*[a-z0-9])?$/i.test(value)) {
                if (window.logger) {
                    window.logger.warn(`[Router] Invalid ${paramName} format:`, value);
                }
                return false;
            }
            ...
            if (value.length < 1 || value.length > maxLength) {
                if (window.logger) {
                    window.logger.warn(`[Router] ${paramName} length invalid:`, value.length);
                }
                return false;
            }
        ...
        if (value.includes('..') || value.includes('<') || value.includes('>') || value.length > maxLength) {
            if (window.logger) {
                window.logger.warn('[Router] Invalid parameter:', value);
            }
            return false;
        }
    ...
    updateGameMetaTags(game) {
        ...
        // this.updateLinkTag('canonical', 'href', `https://kloopik.com/catalog/${game.slug}/`);
    }
}
